    ...
    func createAndAssignAction(_ actionParams:HyperTrackActionParams, _ completionHandler: @escaping (_ action: HyperTrackAction?, _ error: HyperTrackError?) -> Void) {
        ...
        self.requestManager.createAndAssignAction(action) { (action, error) in
            if error == nil && action != nil {
                ...
                nc.post(name:Notification.Name(rawValue:HTConstants.HTActionCreatedNotification),
                        object: nil,
                        userInfo: nil)
                
                if let place =  action?.expectedPlace {
                    Transmitter.sharedInstance.locationManager.startMonitoringForEntryAtPlace(place: place, radius: 200, identifier: action?.id ?? "")
                }
